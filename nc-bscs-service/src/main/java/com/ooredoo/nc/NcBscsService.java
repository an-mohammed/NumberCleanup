...
public class NcBscsService implements ApplicationConstants {
	
	public static ConfigurableApplicationContext context = null;
	private static final Logger LOGGER = LoggerFactory.getLogger(NcBscsService.class);
	...
	public static void main(String[] args) {
		try {
			...
			LOGGER.error("Error occurred while starting-up bscs-cleanup-service. Cause-", e);
	...
	private static void initApplicationFramework(String[] args) throws ApplicationException {
		try {
			context = SpringApplication.run(NcBscsService.class, args);
			context.registerShutdownHook();
			ApplicationContextHolder.getInstance().setApplicationContext(BSCS, context);
		} catch(Exception e ) {
