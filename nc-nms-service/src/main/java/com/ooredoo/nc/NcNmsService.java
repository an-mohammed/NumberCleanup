...
public class NcNmsService implements ApplicationConstants {
	
	public static ConfigurableApplicationContext context = null;
	private static final Logger LOGGER = LoggerFactory.getLogger(NcNmsService.class);
	...
	public static void main(String[] args) {
		try {
			...
			LOGGER.error("Error occurred while starting-up nms-cleanup-service. Cause-", e);
	...
	private static void initApplicationFramework(String[] args) throws ApplicationException {
		try {
			context = SpringApplication.run(NcNmsService.class, args);
			context.registerShutdownHook();
			ApplicationContextHolder.getInstance().setApplicationContext(NMS, context);
		} catch(Exception e ) {
