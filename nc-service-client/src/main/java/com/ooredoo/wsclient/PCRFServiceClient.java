...
@Configuration
public class PCRFServiceClient extends WebServiceGatewaySupport implements WebServiceMessageCallback {

	@Autowired
	private ReloadableResourceBundleMessageSource externalConfig;
	
	private static final Logger LOGGER = LoggerFactory.getLogger(PCRFServiceClient.class);
	...
	public synchronized Object callWebService(String url, Object request, String contextPathForMarshaller) throws WebServiceIOException{
		...
		getWebServiceTemplate().setMessageSender(pcrfMessageSender());
	 ...
	 @Bean
	    public HttpComponentsMessageSender pcrfMessageSender() {
	        HttpComponentsMessageSender messageSender = new HttpComponentsMessageSender();
	        messageSender.setCredentials(
	        		new UsernamePasswordCredentials(externalConfig.getMessage("service.cleanup.pcrf.username", null, Locale.getDefault())
	        				, externalConfig.getMessage("service.cleanup.pcrf.password", null, Locale.getDefault())));
